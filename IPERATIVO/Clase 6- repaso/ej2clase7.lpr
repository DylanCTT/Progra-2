...
function encontrarISBN (a:arbol; ISBN:integer):boolean;

begin
  if(a=nil) then
    encontrarISBN:= false
  else if (a^.datos.ISBN =ISBN) thein
       encontrarISBN:=true
  else if (a^.datos.ISBN<ISBN)then
      encontrarISBN:=encontrarISBN(a^.HI,ISBN)
  else
      encontrarISBN:=encontrarISBN(a^.HD,ISBN);
  end;
